...
export default function AdminContentEdit() {
  ...
  if (!item) {
    return (
      <div className="flex flex-col items-center justify-center" style={{ paddingTop: 80 }}>
        <p style={{ fontSize: 16, color: '#999' }}>Content not found</p>
        <Link to="/admin/content" className="no-underline" style={{ color: '#7a9b7a', fontWeight: 600, marginTop: 12 }}>
          Back to content
        </Link>
      </div>
    );
  ...
  return <EditForm item={item} onSave={(updates) => {
    updateContent(item.id, updates);
    showToast(`"${updates.title || item.title}" updated`);
    navigate('/admin/content');
  }} />;
}

function EditForm({ item, onSave }: { item: ContentItem; onSave: (updates: Partial<ContentItem>) => void }) {
  const navigate = useNavigate();
  ...
  const [selectedTopics, setSelectedTopics] = useState<string[]>(item.topics);
  const [fileName, setFileName] = useState('');
  const [lessons, setLessons] = useState<ProgramLesson[]>(item.lessons || []);
  const fileInputRef = useRef<HTMLInputElement>(null);

  const isProgram = type === 'Program';
  const hasFile = type === 'Audio' || type === 'Video' || type === 'Resource';

  // Non-program content that could be added to a program
  ...
  const handleSave = () => {
    ...
    if (isProgram) {
      ...
      updates.lessonsTotal = lessons.length;
    }
    onSave(updates);
  };
    ...
    <div className="flex flex-col" style={{ gap: 0 }}>
      ...
      <div className="flex" style={{ gap: 40 }}>
        ...
        <div className="flex-1 flex flex-col" style={{ gap: 24, minWidth: 0 }}>
          ...
          {/* File upload for Audio/Video/Resource */}
          {hasFile && (
            <div>
          ...
          {isProgram && (
            <div>
                  ...
                  {lessons.map((lesson, i) => (
                    ...
                    >
                      ...
                      <div className="flex-1" style={{ minWidth: 0 }}>
                        <p className="truncate" style={{ fontSize: 14, fontWeight: 500, color: '#222' }}>
                          {lesson.title}
                        </p>
                      ...
                      <div className="flex items-center shrink-0" style={{ gap: 4 }}>
                        <button
                          ...
                          disabled={i === 0}
                          className="flex items-center justify-center border-none cursor-pointer transition disabled:opacity-20"
                          style={{ width: 28, height: 28, borderRadius: 6, backgroundColor: '#f5f5f5' }}
                        >
                          <svg width="10" height="6" viewBox="0 0 10 6" fill="none">
                            <path d="M1 5L5 1L9 5" stroke="#555" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
                          </svg>
                        </button>
                        <button
                        ...
                        <button
                          ...
                          className="flex items-center justify-center border-none cursor-pointer transition hover:bg-red-50"
                          style={{ width: 28, height: 28, borderRadius: 6, backgroundColor: '#f5f5f5' }}
                        >
              ...
              {/* Add lesson from existing content */}
        ...
        <div className="shrink-0 hidden md:flex flex-col" style={{ width: 300, gap: 24 }}>
          ...
          >
            ...
            <Link
              to={`/content/${type.toLowerCase()}/${item.id}`}
              className="no-underline hover:underline"
              style={{ fontSize: 14, color: '#7a9b7a', fontWeight: 500 }}
            >
              View on site &rarr;
            </Link>
          </div>
      ...
      {isOpen && (
        ...
        >
          ...
          <div
            ...
            style={{ maxHeight: 240, padding: 8, marginTop: 4 }}
          >
            {filtered.length === 0 && (
              <p style={{ padding: '12px 8px', fontSize: 13, color: '#999', textAlign: 'center' }}>
          ...
          <div style={{ padding: '8px 12px 12px', borderTop: '1px solid #f0f0f0' }}>
            <button
              ...
              style={{ fontSize: 13, color: '#999', backgroundColor: 'transparent', padding: 4 }}
            >
              Close
            </button>
          </div>
