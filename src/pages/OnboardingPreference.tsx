...
const options = ['Video', 'Audio', 'Reading', 'A mix of everything'];
...
export default function OnboardingPreference() {
  ...
  const handleNext = () => {
    setOnboardingData({ contentPreference: selected });
    ...
    <div className="bg-bg flex justify-center min-h-screen">
      <div className="flex flex-col items-center gap-6" style={{ width: 480, paddingTop: 140 }}>
        ...
        <p className="font-sans text-center" style={{ fontSize: 16, color: '#6c6c6c', lineHeight: '24px' }}>
          We'll prioritize this format in your feed.
        </p>
          ...
          {steps.map((step) => (
            <div
              ...
              className={`w-2 h-2 rounded-full ${
                step <= 4 ? 'bg-brand' : 'bg-border-medium'
              }`}
            />
          ))}
        </div>
